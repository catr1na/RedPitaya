...
int one_window_filter(float* ch_in, float** ch_out) {
	float* ch_o = *ch_out;
	for(int i=0; i<SPECTR_FPGA_SIG_LEN; i++) {
		ch_o[i] =  ch_in[i] * ((float) rp_window[i]);
	}
	return 0;
}
...
int rp_spectr_int_fft(int16_t* ch_in, int16_t** ch_out) {
	int16_t* ch_o = *ch_out;
	if(!ch_in || !*ch_out)
		return -1;
	if(!rp_kiss_fft_out1 || !rp_kiss_fft_cfg) {
		fprintf(stderr, "rp_spect_fft not initialized");
		return -1;
	}
	fix_fftr(ch_in, 14,0);
	int len = rp_get_spectr_out_signal_length();
	for(int i=0; i<len; i++) {
		//ch_o[i] = (int16_t)pow(rp_kiss_fft_out1[i].r>>8,2)
		//	    + (int16_t) pow(rp_kiss_fft_out1[i].i>>8,2);
		ch_o[i] = sqrt((int)ch_in[i]*ch_in[i]+(int)ch_in[i+256]*ch_in[i+256]);
		if(i<100) {printf("%d\n", ch_o[i]);}
		//ch_o[i] = rp_kiss_fft_out1[i].i*rp_kiss_fft_out1[i].i;
	}
	return 0;
}
