...
int rp_spectr_int_fft(int16_t* ch_in, int** ch_out) {
	int* ch_o = *ch_out;
	if(!ch_in || !*ch_out)
		return -1;
	...
	kiss_fftr(rp_kiss_fft_cfg, (kiss_fft_scalar*) ch_in, rp_kiss_fft_out1);
	len = rp_get_spectr_out_signal_length();
	for(int i=0; i<len; i++) {
		ch_o[i] = (int)pow(rp_kiss_fft_out1[i].r,2) + (int) pow(rp_kiss_fft_out1[i].i,2);
	}
	return 0;
}
